...
export const CineQScorePage = () => {
    ...
    const run = async () => {
        ...
        if (start >= todayStr(0)) {
            toast.error("씨네큐는 상영일 당일 조회가 불가합니다. (전일까지)");
            return;
        }
        setLoading(true);
        setResult(null);
        try {
            const res = await AxiosPost("crawler/cineq_score_all", {
                start,
                end: end || start,
                includes: include,
                excludes: exclude,
    ...
    const openSettings = () => {
        openModal(<CineQAccountSettings />, {
            title: "씨네큐 배급사 계정 설정",
            width: "920px",
        });
    };
        ...
        <Wrapper>
            <Header>
                <HeaderText>
                    <h2>씨네큐 스코어 수집</h2>
                    <p>
                        상영일 기준으로 모든 배급사 계정에 로그인해 전 극장의 관객현황을
                        크롤하고, 배급사별 엑셀로 추출합니다. (상영일 당일 조회 불가 —
                        전일 데이터부터 수집 가능)
                    </p>
                </HeaderText>
            ...
            {loading && (
                <Notice>전 배급사 로그인·전 극장 크롤 중입니다… (보통 수 초 소요)</Notice>
            )}
