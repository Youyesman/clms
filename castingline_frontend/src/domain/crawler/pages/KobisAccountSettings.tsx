...
const emptyDraft: IDraft = { name: "", user: "", password: "", aprv_no: "" };

export const KobisAccountSettings = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const res = await AxiosGet("crawler/kobis_accounts/");
    ...
    const addAccount = async () => {
        ...
        try {
            await AxiosPost("crawler/kobis_accounts", newDraft);
    ...
    const startEdit = (a: IAccount) => {
        setEditId(a.id);
        setEditDraft({ name: a.name, user: a.user, password: a.password, aprv_no: a.aprv_no });
    };
    ...
    const saveEdit = async (id: number) => {
        ...
        try {
            await AxiosPatch("crawler/kobis_accounts", editDraft, id);
    ...
    const toggleActive = async (a: IAccount) => {
        try {
            await AxiosPatch("crawler/kobis_accounts", { is_active: !a.is_active }, a.id);
            load();
        } catch {
    ...
    const removeAccount = (a: IAccount) => {
        showAlert(
            ...
            async () => {
                try {
                    await AxiosDelete("crawler/kobis_accounts", a.id);
    ...
    return (
        <Wrapper>
            <Intro>
                <p>
                    KOBIS 회원용통계 수집에 사용할 배급사 로그인 계정을 관리합니다.
                    <b> 활성화</b>된 계정만 수집 대상에 포함됩니다. (인증번호는 KOBIS
                    로그인 화면의 SMS 인증번호 칸에 쓰는 값 — 없으면 비워두세요)
                </p>
                ...
                <tbody>
                    ...
                    <tr className="addrow">
                        ...
                        <td>
                            <input
                                placeholder="인증번호(선택)"
                                value={newDraft.aprv_no}
                                onChange={(e) =>
                                    setNewDraft({ ...newDraft, aprv_no: e.target.value })
                                }
                    ...
                    {accounts.map((a) =>
                        editId === a.id ? (
                            <tr key={a.id} className="editing">
                                ...
                                <td>
                                    <input
                                        value={editDraft.aprv_no}
                                        onChange={(e) =>
                                            setEditDraft({
                                                ...editDraft,
                                                aprv_no: e.target.value,
                                            })
                                        }
