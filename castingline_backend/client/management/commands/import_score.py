    ...
    def handle(self, *args, **options):
        ...
        with open(options['csv_file'], newline='', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                # Score 객체 생성 (DB에 바로 저장하지 않음)
                score = Score(
                    client=client,
                    movie=movie,
                    entry_date=parse_date(row.get("show_ymd")),
                    auditorium=row.get("screen"),
                    fare=row.get("fee_code"),
                    show_count=row.get("show_num"),
                    visitor=row.get("show_visitor")
                )
                ...
                if len(scores_to_create) >= batch_size:
                    ...
                    created_count += len(scores_to_create)
                    scores_to_create = []  # 리스트 초기화
        ...
        self.stdout.write(self.style.SUCCESS(
            f'{created_count} scores imported successfully!'))
